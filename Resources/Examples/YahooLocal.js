...
(function(){
	...
	var yahooLocal = require('SoupProviders/YahooLocal/soup.yahoolocal');
	...
	yahooLocal.contentSetup(myAPIKey);
	
	//Or you can load your API Key information from a file
	//This file path is from the root of your project's Resources folder
	var myConfigFile = '/SoupProviders/YahooLocal/apikeys.json';
	yahooLocal.contentSetupFromByFile(myConfigFile);
	...
	function myCoordinateCallback(results){
		if(!results.success){
			...
			var myProviderSearchCriteria = yahooLocal.buildSearchCriteria(mySearch);
			
			//We are now ready to search. We just pass in the criteria and a callback
			yahooLocal.searchContent(myProviderSearchCriteria,mySearchCallback);
		}
	};
