...
(function(){
	...
	//Bring in the yelp commonjs library
	var yelp = require('SoupProviders/Yelp/soup.yelp');
	
	//We now need to setup the Yelp library with our API Key information
	...
	function myCoordinateCallback(results){
		if(!results.success){
			...
			var myProviderSearchCriteria = yelp.buildSearchCriteria(mySearch);
			
			//We are now ready to search. We just pass in the criteria and a callback
			yelp.searchContent(myProviderSearchCriteria,mySearchCallback);
		}
	};
